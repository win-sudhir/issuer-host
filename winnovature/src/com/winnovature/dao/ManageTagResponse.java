...
public class ManageTagResponse 
{
	...
	{
		/*XMLData = "<?xml version='1.0' encoding='UTF-8'?><etc:RespMngTagEntries xmlns:etc='http://npci.org/etc/schema/'> " 
		...
		*/
		...
		try
		{
		        ...
		        log.info ("ManageTagResponse.java :: Root element of the response is :: " + doc.getDocumentElement().getNodeName());
		}
		catch (Exception e) 
		{
	...
	public String mngTagApiCall(String tid, String regNum)throws Exception
	//(String tagId, String tid, String vehicleClass, String regNum, String comVehicle) throws Exception
	{
		...
		try
		{
			...
			log.info("ManageTagResponse.java  :::  Exception while ManageTagEntries in mngTagApiCall() : "+e.getMessage());
		...
		if (!tagId.equalsIgnoreCase("NA") || !vehicleClass.equalsIgnoreCase("NA")) 
		{
			...
			log.info("<<<<< ManageTagResponse.java >>>>>  :: "+new SimpleDateFormat("dd-MM-yyyy HH:mm:ss").format(new Date()));
			...
			if (con.getResponseCode() == HttpURLConnection.HTTP_OK) 
			{
				...
				log.info("ManageTagResponse.java ::: mngTagURL : "+mngTagURL+" response code :: "+con.getResponseCode());
				resp = outputResp.toString();
			}
	...
	public boolean allocateTagInsertChallan(String TID, String VehicleNumber, String min_threshold, String txnID, String seqNO,
			String amtIssuence, String amtRecharge, String amtRSA, String amtSecurity, String amtInsurance)
	{
		Connection conn = null;
		CallableStatement callableStatement = null;
		boolean check = false;
		try {
			//String iinNo = NetcUtils.getPropertyValue("iinNNSB");//"609977";//NetcUtils.getPropertyValue("iinNNSB"); //"609977";//NetcUtils.getPropertyValue("iinNNSB");
			...
			if(iinNo!=null)
			{
				...
				if(check)
					log.info("allocateTagInsertChallan PROCEDURE insert challan and updates inventory, vehicle_tag_linking");
				else
					log.info("allocateTagInsertChallan PROCEDURE could not insert challan and updates inventory, vehicle_tag_linking");
				return check;
			...
			//System.out.println("tagId : "+tagId+" vehicleClass : "+vehicleClass+" comVehicle : "+comVehicle);
		} catch (Exception e) {
